...
uniform float mod1 = 0.8; // smaller jitter gives less errors in F2
...
vec2 cellular2x2x2(vec3 P) {
	...
	vec4 p = permuteVec4(Pi.x + vec4(0.0, 1.0, 0.0, 1.0));
	p = permuteVec4(p + Pi.y + vec4(0.0, 0.0, 1.0, 1.0));
	vec4 p1 = permuteVec4(p + Pi.z); // z+0
	vec4 p2 = permuteVec4(p + Pi.z + vec4(1.0)); // z+1
	...
	vec4 dx1 = Pfx + mod1*ox1;
	vec4 dy1 = Pfy + mod1*oy1;
	vec4 dz1 = Pf.z + mod1*oz1;
	vec4 dx2 = Pfx + mod1*ox2;
	vec4 dy2 = Pfy + mod1*oy2;
	vec4 dz2 = Pf.z - 1.0 + mod1*oz2;
